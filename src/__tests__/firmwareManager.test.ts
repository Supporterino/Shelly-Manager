...
vi.mock('../utils/firmware', async (importOriginal) => {
  ...
  return { ...actual, pollUntilOnline: mockPollUntilOnline, pollUntilOffline: mockPollUntilOffline }
})

import { useFirmwareManager, extractCurrentVersion } from '../hooks/useFirmwareManager'
...
describe('useFirmwareManager', () => {
  ...
  it('initialises all devices with idle status', () => {
    const { result } = renderHook(() =>
      useFirmwareManager([device1, device2], {}),
    )
  ...
  it('initialises currentVersion from sys.fw_id when liveStatus is provided', () => {
    ...
    const { result } = renderHook(() =>
      useFirmwareManager([device1], liveStatuses),
    )

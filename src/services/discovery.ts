 ...
 */
export async function verifyShellyHost(ip: string, port = 80): Promise<ShellyInfo | null> {
  try {
  ...
  await Promise.all(
    uniqueHosts.map(async (host) => {
      ...
      try {
        ...
        const [info, status] = await Promise.all([client.getDeviceInfo(), client.getStatus()]);

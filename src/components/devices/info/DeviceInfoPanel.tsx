...
export function DeviceInfoPanel({
  ...
  const genLabel =
    device.generation === 'gen2' ? 'Gen 2' : device.generation === 'gen3' ? 'Gen 3' : 'Gen 4';

  const lastSeenLabel = formatRelativeTime(Date.now() - device.lastSeenAt, i18n.language);
  ...
  return (
    <Stack gap={6}>
      ...
      <InfoRow
        ...
        value={deviceInfoLoading ? <Skeleton height={14} width={80} /> : (deviceInfo?.ver ?? '—')}
      />
      <InfoRow
        ...
        value={
          deviceInfoLoading ? (
            ...
            <Badge size="sm" color={deviceInfo.auth_en ? 'orange' : 'gray'} variant="light">
      ...
      {ramFree != null && ramSize != null && (
        <InfoRow label={t('info.ramFree')} value={`${formatKb(ramFree)} / ${formatKb(ramSize)}`} />
      )}

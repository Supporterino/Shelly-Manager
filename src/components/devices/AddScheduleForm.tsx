...
export function AddScheduleForm({ device, onAdded, onCancel }: Props) {
  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      return new ShellyClient(device).scheduleCreate({
        enable: true,
        timespec,
        calls: [{ method: 'Switch.Set', params: { id: parseInt(selectedSwitch), on: action === 'on' } }],
      })
    },
    onSuccess: () => {
  ...
  return (
    <Stack gap="xs" p="xs" style={{ borderRadius: 'var(--mantine-radius-md)', background: 'var(--mantine-color-default)' }}>
      ...
      <Stack gap={4}>
        <Text size="xs" c="dimmed">{t('schedule.days')}</Text>

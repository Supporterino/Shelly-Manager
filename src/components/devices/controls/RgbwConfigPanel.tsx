...
import { useRgbwConfig, useRgbwSetConfig } from '../../../hooks/useDeviceControl';
import type { RGBWConfig } from '../../../types/shelly';
...
export function RgbwConfigPanel({ deviceId, rgbwId }: Props) {
  const { t } = useTranslation('devices');
  const { t: tc } = useTranslation('common');
  const { data: config, isLoading } = useRgbwConfig(deviceId, rgbwId);
  const mutation = useRgbwSetConfig(deviceId, rgbwId);

  const [name, setName] = useState('');
  const [initialState, setInitialState] = useState<RGBWConfig['initial_state']>('restore_last');
  ...
  function handleSave() {
    const payload: Partial<Omit<RGBWConfig, 'id'>> = {
      name: name || null,
  ...
  return (
    <Stack gap="md" pb="md">
      ...
      <Select
        ...
        value={initialState}
        onChange={(v) => v && setInitialState(v as RGBWConfig['initial_state'])}
        data={[

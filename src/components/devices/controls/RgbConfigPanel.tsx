...
import { useRgbConfig, useRgbSetConfig } from '../../../hooks/useDeviceControl';
import type { RGBConfig } from '../../../types/shelly';
...
export function RgbConfigPanel({ deviceId, rgbId }: Props) {
  const { t } = useTranslation('devices');
  const { t: tc } = useTranslation('common');
  const { data: config, isLoading } = useRgbConfig(deviceId, rgbId);
  const mutation = useRgbSetConfig(deviceId, rgbId);

  const [name, setName] = useState('');
  const [initialState, setInitialState] = useState<RGBConfig['initial_state']>('restore_last');
  ...
  function handleSave() {
    const payload: Partial<Omit<RGBConfig, 'id'>> = {
      name: name || null,
  ...
  return (
    <Stack gap="md" pb="md">
      ...
      <Select
        ...
        value={initialState}
        onChange={(v) => v && setInitialState(v as RGBConfig['initial_state'])}
        data={[

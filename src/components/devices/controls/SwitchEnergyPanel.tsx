...
export function SwitchEnergyPanel({ sw }: Props) {
  ...
  const elapsed = sw?.timer_started_at != null
    ? Math.floor(Date.now() / 1000) - sw.timer_started_at
    : 0
  const remaining = sw?.timer_duration != null
    ? Math.max(0, sw.timer_duration - elapsed)
    : null
      ...
      <Accordion.Item value="energy">
        ...
        <Accordion.Panel>
          <Stack gap={4}>
            ...
            {sw?.pf != null && (
              <Row label={t('power.powerFactor')} value={sw.pf.toFixed(2)} />
            )}
            {sw?.freq != null && (
            ...
            {sw?.temperature != null && (
              <Row
                ...
                value={`${sw.temperature.tC.toFixed(1)} °C`}
              />
            )}
            {remaining != null && remaining > 0 && (
              <Row
                label={t('controls.autoOffIn', { seconds: remaining })}
                value=""
              />
            )}
          </Stack>
